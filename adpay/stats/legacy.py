...
def calculate_events_payments(campaign_doc, timestamp, payment_percentage_cutoff=0.5):
    ...
    while True:
        ...
        user_budget = yield stats_utils.create_user_budget(campaign_doc, timestamp, uid)
        ...
        for event_type in stats_consts.PAID_EVENT_TYPES:
            ...
            if user_budget[event_type]['share'] > 0:
                user_budget[event_type]['event_value'] = int(min([user_budget[event_type]['default_value'],
                                                                  ...
                                                                  user_budget[event_type]['default_value']]))

        yield stats_utils.update_events_payments(campaign_doc, timestamp, uid, user_budget)
...
def calculate_payments_for_new_users(campaign_doc, timestamp):
    ...
    for uid in uids:
        ...
        while True:
            ...
            if not stats_utils.filter_event(event_doc, campaign_doc, banner_doc):
...
def get_best_user_payments_and_humanity(campaign_doc, timestamp, uid):
    ...
    while True:
        ...
        banner_doc = yield db_utils.get_banner(event_doc['banner_id'])
        if not stats_utils.filter_event(event_doc, campaign_doc, banner_doc) and event_type in stats_consts.PAID_EVENT_TYPES:
            event_payment = stats_utils.get_default_event_payment(event_doc, campaign_doc['max_cpc'], campaign_doc['max_cpm'])
