...
class DBTestCase(db_tests.DBTestCase):
    ...
    def test_user_payment_score(self):
        # No user_id in stats.
        user_payment_score = yield stats_utils.get_user_payment_score("campaign_id", 3600, "user_id1")
        self.assertEqual(user_payment_score, 0)

        # Only user_id in stats.
        yield db_utils.update_user_value("campaign_id", 3600, "user_id1", payment=20, human_score=0.5)
        ...
        yield db_utils.update_user_value("campaign_id", 3600, "user_id2", payment=50, human_score=0.8)

        user_payment_score1 = yield stats_utils.get_user_payment_score("campaign_id", 3600, "user_id1")
        user_payment_score2 = yield stats_utils.get_user_payment_score("campaign_id", 3600, "user_id2")
